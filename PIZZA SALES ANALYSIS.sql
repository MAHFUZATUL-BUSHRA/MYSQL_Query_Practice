...
WITH CTE AS
(SELECT O.PIZZA_ID,O.QUANTITY,P.PRICE, QUANTITY*PRICE AS REVENUE
...
-- CS3 : COUNT ORDERS BY DAY
 ...
 SELECT DATE, COUNT(QUANTITY) AS TOTAL_ORDERS
 FROM ORDERS AS O
...
-- CS5 : MONTHLY PIZZA SALES
 ...
 WITH CT AS (SELECT DATE, COUNT(ORDER_ID) AS SALES
 FROM ORDERS
 GROUP BY 1)
 SELECT 
...
SELECT 
    O.DATE,
   ROUND(SUM(QUANTITY*PRICE),2) AS REV
FROM
    PIZZAS AS P
        LEFT JOIN
    order_details AS OD ON P.PIZZA_ID = OD.PIZZA_ID
...
-- 
SELECT
 DATE, SUM(REV) OVER(ORDER BY DATE) AS CUM_DATE
 FROM
(SELECT 
    O.DATE,
   ROUND(SUM(QUANTITY*PRICE),2) AS REV
